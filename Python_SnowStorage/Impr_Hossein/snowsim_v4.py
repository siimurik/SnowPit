...
# ============================================================
#  Numba-compatible RK4 integration
# ============================================================
@njit
def _interp_numba(data_vec, t_query, dt_data):
    """Linear interpolation of a 1-D array at an arbitrary time point."""
    idx = t_query / dt_data
    idx_low = int(np.floor(idx))
    idx_high = idx_low + 1
    n = len(data_vec)
...
def _dTdt_numba(t, Tv,
                ...
                h_ground, Tfreeze):
    """
    Numba-compatible dT/dt for the three snow layers.

    Parameters
    ----------
    t          : current time [s]
    Tv         : temperature array [T1, T2, T3] [K]
    R_a2s      : air-to-surface thermal resistance [m²K/W]
    q_solar    : solar heat flux reaching snow surface [W/m²]
    q_rain     : rain heat flux [W/m²]
    q_evap     : evaporative heat flux [W/m²]
    temp_data  : 1-D array of air temperatures [°C] at hourly intervals
    soil_temp_data : 1-D array of soil temperatures [°C] at hourly intervals
    dt_data    : time step of input data arrays [s]
    R_12, R_23 : inter-layer thermal resistances [m²K/W]
    Cs_layer   : snow layer heat capacity [J/(m²K)]
    h_ground   : ground interface HTC [W/(m²K)]
    Tfreeze    : freezing temperature [K]
    """
    T1, T2, T3 = Tv[0], Tv[1], Tv[2]

    # Interpolate forcing at time t
    ...
    # Layer 3 (bottom) — Robin BC ground flux
    T_soil_K = _interp_numba(soil_temp_data, t, dt_data) + 273.15
    R_cond      = 1.0 / 1.5        # L_soil / k_soil  (defaults: 1.0 m / 1.5 W/mK)
...
def rk4_step(t, T, dt,
             ...
             h_ground, Tfreeze):
    """
    Numba-JIT RK4 integrator for the three snow-layer temperatures.

    Parameters
    ----------
    t, T, dt   : current time [s], temperature state [K], time step [s]
    R_a2s      : combined air + insulation resistance [m²K/W]
    q_solar    : solar heat flux [W/m²]
    q_rain     : rain heat flux [W/m²]
    q_evap     : evaporative flux [W/m²]
    temp_data  : 1-D array – hourly air temperatures [°C]
    soil_temp_data : 1-D array – hourly soil temperatures [°C]
    dt_data    : data time step [s] (typically 3600)
    R_12, R_23 : inter-layer resistances [m²K/W]
    Cs_layer   : layer heat capacity [J/(m²K)]
    h_ground   : ground HTC [W/(m²K)]
    Tfreeze    : 0 °C in Kelvin

    Returns
    -------
    T_new : updated temperature array [K]
    """
    args = (R_a2s, q_solar, q_rain, q_evap,
            temp_data, soil_temp_data, dt_data,
            R_12, R_23, Cs_layer, h_ground, Tfreeze)

    k1 = _dTdt_numba(t,          T,                args[0], args[1], args[2], args[3],
                     args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11])
    k2 = _dTdt_numba(t + dt/2.0, T + dt*k1/2.0,   args[0], args[1], args[2], args[3],
                     args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11])
    k3 = _dTdt_numba(t + dt/2.0, T + dt*k2/2.0,   args[0], args[1], args[2], args[3],
                     args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11])
    k4 = _dTdt_numba(t + dt,     T + dt*k3,        args[0], args[1], args[2], args[3],
                     args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11])
...
def main():
    ...
    # Pre-convert met_data lists to numpy arrays once (required for Numba rk4_step)
    ...
    for k in range(Nt-1):
        ...
        # RK4 integration (Numba-JIT compiled)
        T_new = rk4_step(
            t, T, dt,
            stepPar["R_a2s"], stepPar["q_solar"], stepPar["q_rain"], stepPar["q_evap"],
    ...
    ax6.plot(days, qground_hist, label='Ground flux', linewidth=1, alpha=0.7)  # NEW
    ...
    ax9.set_ylabel('Temperature [°C] / Flux [W/m²]')
    ax9.set_xlabel('Time [days]')
    ax9.legend(loc='best', fontsize=8)
    ax9.grid(True, alpha=0.3)
    ax9.set_title('Ground Interface')
