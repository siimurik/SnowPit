...
def compute_insulation_resistance_multilayer(T_surf, T_snow, k_eff, dt_substep=10.0):
    """
    Compute effective thermal resistance through multi-layer insulation
    using transient heat conduction (implicit method with TDMA).
    
    Returns: R_ins [m²K/W], T_profile
    """
    if not USE_MULTILAYER_INSULATION:
        # Simple single-layer resistance
        return Hi / k_eff, None
    
    # Use transient solver for multi-layer insulation
    n_substeps = int(3600.0 / dt_substep)  # Substeps within one hour
    dx = dz_ins
    dFo = D_ins * dt_substep / dx**2
    
    # Initialize temperature profile (linear initially)
    if not hasattr(compute_insulation_resistance_multilayer, 'T_ins'):
        compute_insulation_resistance_multilayer.T_ins = np.linspace(T_surf, T_snow, N_ins + 1)
    
    T_n = compute_insulation_resistance_multilayer.T_ins.copy()
    
    # Solve heat equation for multiple substeps
    for _ in range(n_substeps):
        ...
        # Boundary condition at outer surface (fixed temperature)
        b[0] = 1.0
        c[0] = 0.0
        d[0] = T_surf
        ...
        # Boundary condition at inner surface (fixed temperature)
        a[-1] = 0.0
        b[-1] = 1.0
        d[-1] = T_snow
        
        T_n = solve_tdma(a, b, c, d, N_ins + 1)
    
    # Store for next iteration
    compute_insulation_resistance_multilayer.T_ins = T_n.copy()
    
    # Compute effective resistance from heat flux
    q_flux = k_eff * (T_n[0] - T_n[1]) / dx  # Heat flux at outer boundary [W/m²]
    if abs(T_surf - T_snow) > 0.01:
        R_ins = (T_surf - T_snow) / q_flux if abs(q_flux) > 1e-6 else Hi / k_eff
    else:
        R_ins = Hi / k_eff
...
def insulation_step(state_in, forc, p, dt):
    ...
    R_ins, T_profile = compute_insulation_resistance_multilayer(T_surf_approx, T_snow_approx, k_eff)
...
def main():
    ...
    for k in range(Nt-1):
        ...
        forc = {
            "Isolar": Isolar,
            "Prain":  Prain,
            "T_rain": Ta_K,  # Assume rain at air temperature
            "RH":     RH_frac,
            "Ta":     Ta_K,
            "U10":    wind
        }
    ...
    plt.tight_layout()
    plt.savefig('snow_storage_simulation.png', dpi=150, bbox_inches='tight')
    print("  Saved plot: snow_storage_simulation.png")
    plt.show()
