...
! gfortran -O2 snowRC_lsoda.f90 YBER_ODEPACK.f ODEPACK_MODULES.f90 -o lsoda_snow -lopenblas -std=legacy
...
subroutine save_timestep_data(k, t, Temp, q_a, q_surf, q_solar, &
                             q_rain, q_evap, q_ground, &
                             T_hist, qa_hist, qnet_surf_hist, &
                             qsolar_hist, qrain_hist, qevap_hist, &
                             qground_hist, Ta_hist, Isolar_hist, &
                             Prain_hist, Nt)
    implicit none
    integer, intent(in) :: k, Nt
    double precision, intent(in) :: t
    double precision, dimension(3), intent(in) :: Temp
    double precision, intent(in) :: q_a, q_surf, q_solar, q_rain, q_evap, q_ground
    
    ! Explicit-shape arrays (no colon)
    double precision, dimension(Nt,3), intent(inout) :: T_hist
    double precision, dimension(Nt), intent(inout) :: qa_hist, qnet_surf_hist
    double precision, dimension(Nt), intent(inout) :: qsolar_hist, qrain_hist
    double precision, dimension(Nt), intent(inout) :: qevap_hist, qground_hist
    double precision, dimension(Nt), intent(inout) :: Ta_hist, Isolar_hist
    double precision, dimension(Nt), intent(inout) :: Prain_hist
    double precision, external :: Ta_fun, Isolar_fun, Prain_fun
    
    ! Save data
    T_hist(k, :) = Temp
    qa_hist(k) = q_a
    qnet_surf_hist(k) = q_surf
    qsolar_hist(k) = q_solar
    qrain_hist(k) = q_rain
    qevap_hist(k) = q_evap
    qground_hist(k) = q_ground
    Ta_hist(k) = Ta_fun(t)
    Isolar_hist(k) = Isolar_fun(t)
    Prain_hist(k) = Prain_fun(t)
END SUBROUTINE save_timestep_data

SUBROUTINE save_insulation_data(k, InsState, R_ins, W_hist, k_eff_hist, &
                               alpha_hist, Rins_hist, fsat_hist, Nt)
    use types_module
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: k, Nt
    TYPE(InsulationState), INTENT(IN) :: InsState
    DOUBLE PRECISION, INTENT(IN) :: R_ins
    ! Explicit-shape arrays (no colon!)
    DOUBLE PRECISION, DIMENSION(Nt), INTENT(INOUT) :: W_hist, k_eff_hist, alpha_hist
    DOUBLE PRECISION, DIMENSION(Nt), INTENT(INOUT) :: Rins_hist, fsat_hist
...
END SUBROUTINE save_insulation_data

SUBROUTINE save_final_timestep(Nt, t_vec, USE_ADVANCED, InsState, R_ins, &
                               Ta_hist, Isolar_hist, Prain_hist, &
                               W_hist, k_eff_hist, alpha_hist, Rins_hist, fsat_hist)
    use types_module
    IMPLICIT NONE
    INTEGER :: Nt
    DOUBLE PRECISION, DIMENSION(Nt) :: t_vec
    LOGICAL :: USE_ADVANCED
    TYPE(InsulationState) :: InsState
    DOUBLE PRECISION :: R_ins
    DOUBLE PRECISION, DIMENSION(Nt) :: Ta_hist, Isolar_hist, Prain_hist
    DOUBLE PRECISION, DIMENSION(Nt) :: W_hist, k_eff_hist, alpha_hist
    DOUBLE PRECISION, DIMENSION(Nt) :: Rins_hist, fsat_hist
    DOUBLE PRECISION, EXTERNAL :: Ta_fun, Isolar_fun, Prain_fun
...
END SUBROUTINE save_final_timestep
...
! ------------------------------------------------------------
!  MODIFIED TIME LOOP - COMPACT VERSION WITH SOLVER CHOICE
! ------------------------------------------------------------
! In your main program, replace the time loop with:
!
! INTEGER, PARAMETER :: SOLVER_CHOICE = 1  ! 1=RK4, 2=DOPRI5, 3=LSODA
!
! do k = 1, Nt-1
!     t = t_vec(k)
!     t_mid = t + dt / 2.0D0
...
!     if (USE_ADVANCED_INSULATION) then
!         call save_insulation_data(k, InsState, R_ins, W_hist, k_eff_hist, &
!                                  alpha_hist, Rins_hist, fsat_hist)
!     end if
! end do