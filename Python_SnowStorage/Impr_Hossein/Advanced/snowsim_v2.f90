...
contains
    ...
    ! ============================================================
    pure double precision function ground_flux_robin_bc(T3, Tg_deep, h_ground, & 
        ...
        !--------------------------------------------------------------------
        implicit none
        double precision, intent(in) :: T3, Tg_deep, h_ground
...
program main_enhanced
    ...
    double precision :: theta_e, h_ground, Tg_deep, k_soil, L_soil
    ...
    theta_e = 0.04D0        ! UPDATED: was 0.03D0
    h_ground = 4.0D0        ! NEW: Ground HTC [W/m²K]
    Tg_deep = 273.15D0 + 3.0D0  ! NEW: Deep ground temperature [K]
    k_soil = 1.5D0          ! NEW: Soil conductivity [W/mK]
    L_soil = 1.0D0          ! NEW: Soil layer thickness [m]
    Hi = 0.20D0             ! UPDATED: was 0.10D0
    ...
    do k = 1, Nt-1
        ...
        q_ground_mid = ground_flux_robin_bc(Tempe(3), Tg_deep, h_ground)
...
subroutine dTdt(t, Tv, R_a2s, q_solar, q_rain, q_evap, &
            ...
            Tg_deep, h_ground, k_soil, L_soil, dT)
    ! dT/dt for snow layers
    ! Tv = [T1, T2, T3]
    ! R_nm = [R_12, R_23, R_3g]
    ...
    double precision, intent(in) :: Cs_layer, Tg_deep, h_ground, k_soil, L_soil
    ...
    q_32 = (T2 - T3) / R_23
    !q_3g = (Tg - T3) / R_3g
    q_3g = ground_flux_robin_bc(T3, Tg_deep, h_ground, k_soil, L_soil)
...
SUBROUTINE step_RK4(t, dt, T_current, R_eff, R_ins, q_solar, q_rain, &
    ...
    DOUBLE PRECISION, INTENT(IN) :: Tg_deep, h_ground, k_soil, L_soil
    INTEGER, INTENT(IN)          :: itemp_size 
...
SUBROUTINE integrate_DOPRI5(t, dt, T_current, R_ins, R_eff, q_solar, q_rain, &
                            q_evap, R_nm, Cs_layer, Tg_deep, h_ground, k_soil,&
                            L_soil, temp_vec, itemp_size, &
                            dt_data, T_new)
    IMPLICIT NONE
    DOUBLE PRECISION :: t, dt, R_ins, R_eff, q_solar, q_rain, q_evap
    DOUBLE PRECISION :: Cs_layer, Tg_deep, h_ground, k_soil, L_soil, dt_data !, Tg
    INTEGER :: itemp_size
...
SUBROUTINE snow_rhs_dopri5(N, T_TIME, Y, F, RPAR, IPAR)
    ...
    DOUBLE PRECISION :: q_3g, Tg_deep, h_ground, k_soil, L_soil
    INTEGER :: v_size
    ...
    !q_3g = (Tg - Y(3)) / R_3g
    ! NEW:
    q_3g = ground_flux_robin_bc(Y(3), Tg_deep, h_ground, k_soil, L_soil)
...
SUBROUTINE integrate_LSODA(t, dt, T_current, R_ins, R_eff, q_solar, q_rain, &
    ...
    DOUBLE PRECISION :: Tg_deep, h_ground, k_soil, L_soil
    INTEGER :: itemp_size
    DOUBLE PRECISION, DIMENSION(itemp_size) :: temp_vec
    DOUBLE PRECISION, DIMENSION(3) :: T_current, R_nm, T_new
...
SUBROUTINE snow_rhs_lsoda(NEQ, T_TIME, Y, YDOT, common_data)
    ...
    q_32 = (Y(2) - Y(3)) / R_23
    !q_3g = (Tg - Y(3)) / R_3g
    q_3g = ground_flux_robin_bc(Y(3), Tg_deep, h_ground, k_soil, L_soil) ! NEW!
