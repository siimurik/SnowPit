...
# Characteristic length for mesh refinement
char_length = 0.95  # smaller value for finer mesh

# Helper function to add circle points
def add_circle_points(radius, z, num_segments, char_length):
    points = []
    for i in range(num_segments):
        angle = 2 * m.pi * i / num_segments
        x = radius * m.cos(angle)
        y = radius * m.sin(angle)
        points.append(gmsh.model.occ.addPoint(x, y, z, char_length))
    return points

# Add points for the bottom and top circles
bottom_points = add_circle_points(bottom_radius, 0.0, num_segments, char_length)
top_points = add_circle_points(top_radius, height, num_segments, char_length)
