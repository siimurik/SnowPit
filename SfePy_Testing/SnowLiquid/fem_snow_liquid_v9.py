...
Vapor Equation (Vapor Content Cv), Pressure Equation (Pore Pressure P).
    ...
    Heat:     ρcp ∂T/∂t = k_t ∇²T + k_c ∇²Cl + k_p ∇²P + ΔH εᴸ ∂Cl/∂t + q_rain
    Liquid:   ∂Cl/∂t = D_l ∇²Cl + D_t_l ∇²T + D_p_l ∇²P - m_evap
    Vapor:    ∂Cv/∂t = D_v ∇²Cv + D_t_v ∇²T + D_p_v ∇²P + m_evap
    Pressure: ∂P/∂t = α_P ∇²P + α_T ∇²T + α_L ∇²Cl + α_V ∇²Cv
...
def get_rain_heat_flux(ts, coors, mode=None, **kwargs):
    """Sensible heat flux from rainfall"""
    if mode != 'qp' or coors is None:
        return {}
    ...
    if current_temp > 0.0 and current_prec > 0.0:
        # Sensible heat flux from rain: W/m²
        q_rain = current_prec * rho_water * c_water * current_temp / 3600.0
        # Convert to volumetric source: W/m³
        q_rain_vol = q_rain / d_ins  # Distribute over depth
    else:
        q_rain_vol = 0.0
