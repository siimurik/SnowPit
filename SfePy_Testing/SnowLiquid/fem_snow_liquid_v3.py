...
rh_i = 0.5        # Inner relative humidity (constant) [fraction, 0-1]
h_l_i = 1e-6      # Inner liquid mass transfer coefficient [m/s]
h_l_o = 2e-6      # Outer liquid mass transfer coefficient [m/s]

# Humidity-to-liquid conversion parameters
max_liquid_content = 0.05  # Maximum liquid content at 100% RH [kg/m^3]
min_liquid_content = 0.001 # Minimum liquid content at 0% RH [kg/m^3]

time_data, rh_data, t_o, h_o = read_temp_and_hcoeff_from_csv()

# Convert Unix timestamps to simulation time (starting from 0)
if len(time_data) > 0:
    start_timestamp = time_data[0]  # First timestamp becomes t=0
    simulation_times = [t - start_timestamp for t in time_data]
else:
    simulation_times = []
...
def get_h_o(ts, coors, mode=None, **kwargs):
    ...
    # Find the correct time index using simulation time
    time_idx = 0
    ...
    # Handle case where we're beyond available data
    time_idx = min(time_idx, len(h_o) - 1)
    
    val = nm.full((coors.shape[0], 1, 1), h_o[time_idx], dtype=nm.float64)
    return {'val': val}
...
def get_t_o(ts, coors, mode=None, **kwargs):
    ...
    # Handle case where we're beyond available data
    time_idx = min(time_idx, len(t_o) - 1)
    
    val = nm.full((coors.shape[0], 1, 1), t_o[time_idx], dtype=nm.float64)
    return {'val': val}

def humidity_to_liquid_content(rh_percent, T_celsius):
    """
    Convert relative humidity to liquid content reference value.
    ...
    # Simple linear relationship between RH and liquid content
...
def get_cl_o(ts, coors, mode=None, **kwargs):
    """
    Time-dependent outer liquid reference content based on humidity.
    """
    if mode != 'qp' or coors is None:
        return {}
    ...
    # Handle case where we're beyond available data
    time_idx = min(time_idx, len(rh_data) - 1)
    
    # Get current humidity and temperature
    current_rh = rh_data[time_idx]
    current_temp = t_o[time_idx]
    
    # Convert humidity to liquid content reference
    cl_ref = humidity_to_liquid_content(current_rh, current_temp)
    
    val = nm.full((coors.shape[0], 1, 1), cl_ref, dtype=nm.float64)
    return {'val': val}

def get_cl_i(ts, coors, mode=None, **kwargs):
    """
    Inner boundary liquid reference content (constant humidity).
    """
    if mode != 'qp' or coors is None:
        return {}

    # Use constant inner relative humidity
    cl_ref = humidity_to_liquid_content(rh_i * 100, t_i)  # rh_i is in fraction
...
materials = {
    ...
    'h_out_dyn': 'get_h_o',
    'T_out_dyn': 'get_t_o', 
    'Cl_out_dyn': 'get_cl_o',    # Humidity-dependent outer liquid reference
    'Cl_in_dyn': 'get_cl_i',     # Constant inner liquid reference
...
# Coupled equations with humidity-dependent Robin BCs
equations = {
    ...
    'Liquid': """dw_dot.i.Omega(r, dCl/dt)
               ...
               = - dw_volume_lvf.i.Omega(evaporation_rate.val, r)
                 ...
                 - dw_bc_newton.i.Gamma_Right(in_fixed.h_l_in, Cl_in_dyn.val, r, Cl)
               """
}
...
# Time parameters
nr_hour = len(simulation_times)
start = 0.0
nr_of_hours = min(10, nr_hour)  # Use available data or 10 hours, whichever is smaller
...
def save_coupled_results(out, problem, state, extend=False):
    ...
    filename = os.path.join(problem.conf.options['output_dir'], "humidity_coupled_results.csv")
    ...
    if problem.ts.step == 0:
        with open(filename, 'w', newline='') as csvfile:
            ...
            writer.writerow(["Time (s)", "Node Index", "Position (m)", 
                           ...
                           "Outer RH (%)", "Outer Temp (°C)", "Unix Timestamp"])

    # Save data at full-hour marks
    if problem.ts.time % 3600 < problem.ts.dt:  
        # Get current environmental conditions using simulation time
        time_idx = 0
        for i, sim_time in enumerate(simulation_times):
            if problem.ts.time >= sim_time:
                time_idx = i
            else:
                break
        ...
        current_rh = rh_data[time_idx]
        current_temp_out = t_o[time_idx]
        current_unix_time = time_data[time_idx] if time_idx < len(time_data) else 0
        
        with open(filename, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            for i, coord in enumerate(coors):
                writer.writerow([problem.ts.time, i, coord[0], T_vals[i], Cl_vals[i],
                               current_rh, current_temp_out, current_unix_time])
...
options = {
    ...
    'output_dir': './output_snow_humidity',
}